        ...
        if (child.stateObj === TaskState.Completed) {
            if (child.actionObj instanceof CallHttpAction) {
                ...
                if (result.statusCode === 202 && result.headers && result.headers.Location) {
                    const delay: moment.Duration =
                        result.headers && result.headers["Retry-After"]
                    ...
                    const callHttpAction = new CallHttpAction(this.action.httpRequest);
                    const callHttpTask = new AtomicTask(false, callHttpAction);
